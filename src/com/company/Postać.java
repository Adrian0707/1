...
public class Postać extends Obiekty implements Animowane{
    ...
    public Postać(int i){
        ...
        if (i == 1){
            ...
            setY(380);
            setKier(true);
            Wskaznik ws = new Wskaznik(getX() + 100, getY(), root, isKier());
            wsk = ws;
            pasŻ = new Rectangle(20, 20, getZdrowie() * 2, 40);
       /*   pasŻ.setStrokeLineCap(StrokeLineCap.ROUND);
          ...
          pasŻ.setStrokeLineJoin(StrokeLineJoin.ROUND);*/
            ...
            setY(380);
            setKier(false);
            Wskaznik ws = new Wskaznik(getX(), getY(), root, isKier());
            wsk = ws;
            pasŻ = new Rectangle(580, 20, getZdrowie() * 2, 40);
    ...
    public void obrazenie(double dmg, double x, double y){
        ...
        for (int i = 0; i < dmg; i++){
            new Krew(x, y, root, obiekt, Color.DARKRED, random() * 4);
        }
        if (getZdrowie() <= 0)
            smierc();
    ...
    public void smierc(){
        ...
        for (int i = 0; i < 40; i += 2){
            ...
            for (double alf = 0; alf < 360; alf += 360 / 10){
                ...
                new Spark(i * sin(alf) + wsk.xt, i * cos(alf * 1.5) + wsk.yt, Color.DARKRED, random() * 3 + 2, 0, 0.03);
